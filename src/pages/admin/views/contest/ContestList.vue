      ...
      <el-table
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <p>比赛开始时间: {{props.row.start_time | localtime }}</p>
            <p>比赛结束时间: {{props.row.end_time | localtime }}</p>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag :type="scope.row.contest_type === 'Public' ? 'success' : 'primary'">
              {{ scope.row.contest_type==='Public'?'公开':'密码'}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <div slot-scope="scope">
            ...
            <icon-btn name="问题" icon="list-ol" @click.native="goContestProblemList(scope.row.id)"></icon-btn>
            <icon-btn name="公告" icon="info-circle"
                      @click.native="goContestAnnouncement(scope.row.id)"></icon-btn>
            <icon-btn icon="download" name="下载已AC的提交"
